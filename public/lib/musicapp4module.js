...
function selectThePart(part){
  ...
  let pieceId = document.getElementById("lhPieceId").textContent;
  image.src= "../userpieces/" + pieceId + "/media/"+part+".jpg";
  ctx = thePartCanvas.getContext("2d");
  image = document.getElementById("source");
...
function switchPart(part){
  theCurrentPart = part;
  partModel.boxHeight = pieceDataSet.boxSize[part];
  console.log('pieceData)=');
  console.dir(pieceData);
  partModel.timeOffset = partModel.timeOffsets[part];

  animation = pieceDataSet.animation;
  selectThePart(part);
  initPart();
  drawPart();
}
...
function switchPiece(piece){
  ...
  partModel.timeOffsets = pieceDataSet.timeOffsets;

  partModel.notes = notes;
  partModel.position=0;
  partModel.boxHeight = pieceDataSet.boxSize;
